...
export default {
  ...
  Mutation: {
    addTrackToFavourites: async (_, { id }): Promise<IFavorite> => {
      return await addToFavourites(id, 'tracks');
    },
    addBandToFavourites: async (_, { id }): Promise<IFavorite> => {
      return await addToFavourites(id, 'bands');
    },
    addGenreToFavourites: async (_, { id }): Promise<IFavorite> => {
      return await addToFavourites(id, 'genres');
    },
    addArtistToFavourites: async (_, { id }): Promise<IFavorite> => {
      return await addToFavourites(id, 'artists');
    },
    removeTrackToFavourites: async (_, { id }): Promise<IFavorite> => {
      return await removeFromFavourites(id, 'tracks');
    },
    removeBandToFavourites: async (_, { id }): Promise<IFavorite> => {
      return await removeFromFavourites(id, 'bands');
    },
    removeGenreToFavourites: async (_, { id }): Promise<IFavorite> => {
      return await removeFromFavourites(id, 'genres');
    },
    removeArtistToFavourites: async (_, { id }): Promise<IFavorite> => {
      return await removeFromFavourites(id, 'artists');
    },
  },
  Favourite: {
    genres: async ({ genresIds }) => {
      return await Promise.all(genresIds.map((id) => getGenre(id)));
    },
    artists: async ({ artistsIds }) => {
      return await Promise.all(artistsIds.map((id) => getArtist(id)));
    },
    bands: async ({ bandsIds }) => {
      return await Promise.all(bandsIds.map((id) => getBand(id)));
    },
    tracks: async ({ tracksIds }) => {
